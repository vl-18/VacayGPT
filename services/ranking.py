...
def generate_penalties(row):

    penalties = []

    if row["season_score"] < 6:
        penalties.append("Seasonal suitability is moderate for these dates.")

    if row["crowd_match"] < 6:
        penalties.append("Crowd levels may not match your preference.")

    if row["accessibility_score"] < 5:
        penalties.append("Travel access may require longer transit or multiple connections.")

    if row["budget_score"] < 5:
        penalties.append("Estimated daily costs are relatively high.")

    if not penalties:
        penalties.append("No major drawbacks detected for your preferences.")
...
def compute_travel_scores(df, crowd_pref):
    ...
    df["penalties"] = df.apply(generate_penalties, axis=1)
